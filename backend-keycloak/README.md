...
The Maven Wrapper is available at the repository root, so Maven does not need to
be installed globally.

Unless explicitly stated otherwise, the commands below are intended to be run
...
Kafka event publication is disabled by default. When
`IDENTITY_EVENTS_ENABLED=true`, this backend publishes `identity.events` after
user, group and role create/update/delete operations.

In event-enabled mode, mutating requests must have an actor. If OIDC security is
enabled and the request is authenticated, the authenticated principal is used. If
the API is running open, clients must send:
...
Generic notification command publication is disabled by default. When
`NOTIFICATION_COMMANDS_ENABLED=true`,
`POST /api/v1/users/{id}/actions/update-password-email` publishes a generic
email command to `notification.commands` instead of asking Keycloak to send the
email through its own SMTP configuration.

This mode is intentionally Keycloak-independent. The notification service sends
...
| `POST` | `/api/v1/users/{id}/actions/update-password-email` | Send an update-password email through Keycloak or Kafka notification mode. |
...
To send the update-password email, call the explicit action endpoint:

```http
POST /api/v1/users/{id}/actions/update-password-email
```

With `NOTIFICATION_COMMANDS_ENABLED=false`, this endpoint asks Keycloak to send
the `UPDATE_PASSWORD` required action email and returns `204 No Content` when
Keycloak accepts the request. SMTP must be configured in Keycloak for the email
action to succeed. If SMTP is not configured, the API maps the Keycloak error
into the centralized problem response.

With `NOTIFICATION_COMMANDS_ENABLED=true`, this endpoint publishes a generic
email command to Kafka. The notification service sends the e-mail and emits a
notification result event. If Kafka publication fails, the request fails.

Kafka/Testcontainers integration tests can be run from the repository root:

```shell
./mvnw -pl backend-keycloak -DskipITs=false verify
```
...
`POST /api/v1/users/{id}/actions/update-password-email` after creating the user
when the application wants to trigger the email through the configured mode.
...
- unavailable update-password email in Keycloak SMTP mode
- notification command publication in Kafka notification mode
...
```shell
./mvnw -pl backend-keycloak -Duser.language=pt -Duser.country=BR -DskipITs=false verify
```
...
```shell
./mvnw -pl backend-keycloak package
```

Run the generated application:

```shell
java -jar backend-keycloak/target/quarkus-app/quarkus-run.jar
```

Build an uber jar:

```shell
./mvnw -pl backend-keycloak package -Dquarkus.package.jar.type=uber-jar
```

Build a native executable:

```shell
./mvnw -pl backend-keycloak package -Dnative
```

Build native using a container:

```shell
./mvnw -pl backend-keycloak package -Dnative -Dquarkus.native.container-build=true
```
